...
class Base {
    ...
    setOmit(omit) {
        ...
        for (let [index, item] of omit.entries()) {
            me.omit.set(index, item)
        }
        $(me.omit_el).each(function(index, item) {
            $(item).text(me.omit.get(index))
        })
    }
    //设置开奖号码
    setOpenCode(code) {
        ...
        for (let item of code.values()) {
            me.open_code.add(item)
        }
        me.updateOpenCode && me.updateOpenCode.call(me, code)
    }
    //号码选中取消
    toggleCodeActive(e) {
        let me = this
    ...
    changePlayNav(e) {
        ...
        me.cur_play = $cur.attr('desc').toLocaleLowerCase()
        $('#zx_sm span').html(me.play_list.get(me.cur_play).tip)
    ...
    addCode() {
        ...
        let count = me.computeCount(active, me.cur_play)
        if (count) {
            me.addCodeItem($active.join(' '), me.cur_play, me.play_list.get(me.cur_play).name, count)
        }
    }
    //添加单次号码
    ...
    getCount() {
        ...
        let count = me.computeCount(active, me.cur_play);
        let range = me.computeBonus(active, me.cur_play);
        let money = count * 2;
    ...
    getRandomCode(e) {
        ...
        if (num === '0') {
            ...
            for (let i = 0; i < num; i++) {
                me.addCodeItem(me.getRandom(play), me.cur_play, me.play_list.get(me.cur_play).name, 1)
            }
        }
    }
}
